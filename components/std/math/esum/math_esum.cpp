...
________________________________________________________________
	...
	$Id:: math_esum.cpp 2428 2009-12-11 15:10:37Z benjmitch    $
	$Rev:: 2428                                                $
	$Author:: benjmitch                                        $
...
#define COMPONENT_CLASS_STRING "std/2009/math/esum"
#define COMPONENT_CLASS_CPP std_2009_math_esum_0
...
template <class T> void addValue(T* outReal, T* outImag, T* inReal, T* inImag, UINT32 no, UINT32 ni)
{
	//	scalar
	if (ni == 1)
	{
...
Symbol COMPONENT_CLASS_CPP::event(Event* event)
{
	switch(event->type)
	{
		...
		case EVENT_STATE_GET:
		{
			...
			//	set field
			if (!state.hasField("complex"))
			{
				Vbool vcpx;
				vcpx.push_back(complex);
				state.addField("complex").setArray(Dims(1), vcpx);
			}

			//	set field
			if (!state.hasField("dims"))
				state.addField("dims").setArray(Dims(1, dims.size()), dims);

			//	set field
			if (!state.hasField("type"))
			{
				string s = getElementTypeString(dataType);
				state.addField("type").setSTRING(s.c_str());
			}
...
////////////////	IF NO INPUTS, OUTPUT IS ZERO

			//	if no inputs
			if (!inputs.size())
			{
				//	just use memset, for speed
				memset(&outputBuffer.buffer[0], 0, outputBuffer.buffer.size());
			}
...
////////////////	FOR ANY FURTHER INPUTS, OUTPUT IS ADDED TO THEM

			//	for each further input, add it to the output
			for (UINT32 i=1; i<inputs.size(); i++)
			{
				...
				{
					case TYPE_DOUBLE:
						addValue((DOUBLE*)outputBuffer.real, (DOUBLE*)outputBuffer.imag, (DOUBLE*)input.real, (DOUBLE*)input.imag, numEls, iNumEls);
						break;
					case TYPE_SINGLE:
						addValue((SINGLE*)outputBuffer.real, (SINGLE*)outputBuffer.imag, (SINGLE*)input.real, (SINGLE*)input.imag, numEls, iNumEls);
						break;
					case TYPE_UINT64:
						addValue((UINT64*)outputBuffer.real, (UINT64*)outputBuffer.imag, (UINT64*)input.real, (UINT64*)input.imag, numEls, iNumEls);
						break;
					case TYPE_UINT32:
						addValue((UINT32*)outputBuffer.real, (UINT32*)outputBuffer.imag, (UINT32*)input.real, (UINT32*)input.imag, numEls, iNumEls);
						break;
					case TYPE_UINT16:
						addValue((UINT16*)outputBuffer.real, (UINT16*)outputBuffer.imag, (UINT16*)input.real, (UINT16*)input.imag, numEls, iNumEls);
						break;
					case TYPE_UINT8:
						addValue((UINT8*)outputBuffer.real, (UINT8*)outputBuffer.imag, (UINT8*)input.real, (UINT8*)input.imag, numEls, iNumEls);
						break;
					case TYPE_INT64:
						addValue((INT64*)outputBuffer.real, (INT64*)outputBuffer.imag, (INT64*)input.real, (INT64*)input.imag, numEls, iNumEls);
						break;
					case TYPE_INT32:
						addValue((INT32*)outputBuffer.real, (INT32*)outputBuffer.imag, (INT32*)input.real, (INT32*)input.imag, numEls, iNumEls);
						break;
					case TYPE_INT16:
						addValue((INT16*)outputBuffer.real, (INT16*)outputBuffer.imag, (INT16*)input.real, (INT16*)input.imag, numEls, iNumEls);
						break;
					case TYPE_INT8:
						addValue((INT8*)outputBuffer.real, (INT8*)outputBuffer.imag, (INT8*)input.real, (INT8*)input.imag, numEls, iNumEls);
						break;
					default:
						berr << "not coded yet for this data type";
				}
