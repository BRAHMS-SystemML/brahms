...
/*
	...
	since spikes doesn't currently write to file at run-time,
	we've disabled all the file writing, and it just always
	sends its ouput encapsulated. this may be troublesome when
	large numbers of spikes are generated, so we may have to fix
