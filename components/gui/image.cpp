...
________________________________________________________________
	...
	$Id:: image.cpp 2426 2009-12-10 21:10:50Z benjmitch        $
	$Rev:: 2426                                                $
	$Author:: benjmitch                                        $
...
#define COMPONENT_CLASS_STRING "dev/std/image/numeric"
#define COMPONENT_CLASS_CPP dev_std_image_numeric_0
...
//	TODO: we should do this dynamic-linking, no point in having extra copies of WX in memory
#include "wx/wx.h"
...
//		cout << "MyApp::OnExit()" << endl;
	...
	bool Unlock(INT32 hWindow)
	{
		return windows[hWindow]->Unlock();
	}
...
BOOL APIENTRY DllMain(HANDLE hModule, DWORD reason, LPVOID lpReserved)
{
	switch (reason)
	{
		case DLL_PROCESS_ATTACH:
			if (!wxInit())
			{
				cerr << __FILE__ << ":" << __LINE__ << " failed initialize wxWidgets" << endl;
				return FALSE;
			}
			break;
	...
	switch(event->type)
	{
		...
		case EVENT_INIT_CONNECT:
		{
			...
			if (event->flags & F_LAST_CALL)
			{
				...
				input.validateStructure(TYPE_REAL);

				//	caption is input name, if not otherwise specified
				if (!caption.length())
					caption = input.getName();
			...
			////////	COLORMAP MODE
				...
				else
				{
					...
					switch(inputType)
					{
						...
						case TYPE_UINT64:
							transform(((UINT64*)p_input) + base, p_rgb,
								transform_offset, transform_range,
								elements[0], elements[1], strides[0], strides[1],
								&colormap[0]);
							break;
						...
						case TYPE_INT64:
							transform(((INT64*)p_input) + base, p_rgb,
								transform_offset, transform_range,
								elements[0], elements[1], strides[0], strides[1],
								&colormap[0]);
							break;
				...
				//	access target
				if (!wxGetApp().Unlock(hWindow))
					bout << "FAILED WAIT (" << componentData->name << ")" << D_WARN;
