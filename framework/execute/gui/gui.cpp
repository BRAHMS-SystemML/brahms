...
////////////////	GUI CLASS IMPLEMENTATION
...
#ifdef __NIX__
		if (displayed) XtDestroyApplicationContext(app);
	...
	Symbol ExecuteGUI::MonitorEventHandlerFunc(const MonitorEvent* event)
	{
		...
		/*
				...
				right.bottom -= metric_VerticalSpacing;
				gui.infoPanel = (UINT32)CreateWindowEx(
					0L, "EDIT", "",
					WS_CHILD | WS_VISIBLE | WS_EX_OVERLAPPEDWINDOW | ES_LEFT | ES_MULTILINE,
					right.left, right.top, right.right, right.bottom,
					(HWND)gui.window, NULL, (HINSTANCE) gui.instance, NULL
				);
		*/
...
/*
				if (event->createEngine->voiceCount == 1)
				{
					//	position in centre
					SetWindowPos((HWND)gui.window, NULL,
						WorkArea.right/2 - metric_WindowWidth/2, WorkArea.bottom/2 - metric_WindowHeight/2,
						metric_WindowWidth, metric_WindowHeight, SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_NOZORDER | SWP_NOREDRAW);
				}
				else
				{
					//	if enough room, list them in the middle
					UINT32 perwindow = metric_WindowHeight + metric_VerticalSpacing;
					UINT32 allwindows = perwindow * event->createEngine->voiceCount;
					if (allwindows <= WorkArea.bottom)
					{
						UINT32 top = WorkArea.bottom/2 - (perwindow * event->createEngine->voiceCount / 2);
						SetWindowPos((HWND)gui.window, NULL,
							WorkArea.right/2 - metric_WindowWidth/2, top + perwindow * event->createEngine->voiceIndex,
							metric_WindowWidth, metric_WindowHeight, SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_NOZORDER | SWP_NOREDRAW);
					}

					//	stagger across screen, but based around the centre
					else
					{
						UINT32 y_lowest = WorkArea.bottom - metric_WindowHeight - metric_VerticalSpacing;
						UINT32 y_highest = metric_VerticalSpacing;
						UINT32 y_offset = (y_lowest - y_highest) / (event->createEngine->voiceCount - 1);
						SetWindowPos((HWND)gui.window, NULL,
							WorkArea.right/2 - metric_WindowWidth/2, y_highest + y_offset * event->createEngine->voiceIndex,
							metric_WindowWidth, metric_WindowHeight, SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_NOZORDER | SWP_NOREDRAW);
					}
				}
				*/
			...
			{
/*
					"brahms.*.font: -*-helvetica-medium-r-normal-*-16-*-*-*-*-*-*-*",
					"brahms.*.font: -*-courier-medium-r-normal-*-14-*-*-*-*-*-*-*",
					"brahms.*.font: 7x14",
					"brahms.*.form.displayList: line " xawWidgetLine, // this puts a line on the GUI form!
					*/
					
				//	fallback resources
				const char*	fallback_resources[] = {
					"brahms*font: -*-arial-medium-r-normal-*-12-*-*-*-*-*-*-*",
//					"brahms*phase*font: -*-arial-bold-r-normal-*-12-*-*-*-*-*-*-*",
				...
				//	create form
				gui.form = XtVaCreateManagedWidget("form", formWidgetClass, gui.application,
					/* XtNdisplayList, "draw-line", something to draw the center line... */
					NULL);
...
/*
				//	get handle to form's window so we can draw on it during update
				gui.window = XtWindow(gui.form);
				if (!gui.window)
				{
					____WARN("X GUI: 'window' is zero" << endl)
					break;
				}
				
				//	position it
				XMoveWindow(gui.display, gui.window, left, top);
*/

/*
				
				//	wait for expose event on the window
				XEvent xevent;
		        XSelectInput(gui.display, gui.window, ExposureMask);
		        XNextEvent(gui.display, &xevent);
		        */
		        
		        /*
				
				//	get GC				
				gui.context = XCreateGC(gui.display, gui.window, 0, NULL);
				if (gui.context < 0)
				{
					____WARN("X GUI: 'context' is invalid");
					break;
				}
				*/
...
/*				
				//	get colors
				if (DisplayPlanes(gui.display, gui.screen) != 1)
				{
					Colormap cmap = DefaultColormap(gui.display, gui.screen);
					XColor color, colorrgb;
					if (XAllocNamedColor(gui.display, cmap, "red", &color, &colorrgb))
					{
						XSetForeground(gui.display, gui.context, color.pixel);
					}
				}
				*/
			...
			case EVENT_MONITOR_OPERATION:
			{
				...
				//	just update message
				//MonitorTime* t = event->monitorTime;
				//double secsElapsed = t->wallclockTime;
				//string msg = "Wall Clock: " + prettyPrintTime(secsElapsed, false) + "\n";
				//msg += event->message;
...
/*
		if (gui.context >= 0)
		{
			XFillRectangle(gui.display, gui.window, gui.context,
				0, 0, 100, 100);
		}
*/
...
/*
			...
			HGDIOBJ old = SelectObject(gui.deviceContext, GetStockObject(BLACK_PEN));
			MoveToEx(gui.deviceContext, gui.center.left - metric_HorizontalSpacing, gui.center.top, NULL);
			LineTo(gui.deviceContext, gui.center.left - metric_HorizontalSpacing, gui.center.bottom);
			//MoveToEx(gui.deviceContext, gui.right.left - metric_HorizontalSpacing, gui.right.top, NULL);
			//LineTo(gui.deviceContext, gui.right.left - metric_HorizontalSpacing, gui.right.bottom);
			SelectObject(gui.deviceContext, old);
*/
