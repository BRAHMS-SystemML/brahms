...
%__________________________________________________________________________
%
% $Id:: removesubsystem.m 2397 2009-11-18 21:20:07Z benjmitch            $
% $Rev:: 2397                                                            $
% $Author:: benjmitch                                                    $
...
function sys = removesubsystem(sys, varargin)

name = varargin{1};

for n = 1:length(sys.sub)
	if strcmp(sys.sub{n}.name, name)
		sys.sub = sys.sub([1:n-1 n+1:end]);
		sys.namesinuse = rmfield(sys.namesinuse, name);
		return;
	end
end

error(['sub-system "' name '" not found']);
