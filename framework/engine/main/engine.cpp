...
namespace brahms
{
    ...
    {
        if (sz != esz) {
            ferr << E_INTERNAL << "failed assert during engine init: sizeof("
                 << msg << ") == " << sz << " != " << esz;
        }
    }
        ...
        comms(engineData)
    {
        ...
        if (engineData.core.createEngine.voiceIndex == VOICE_FROM_MPI) {
            ...
            brahms::channel::CommsInitData commsInitData = comms::initChannelModule(engineData,
                                                                                    brahms::channel::PROTOCOL_MPI,
                                                                                    commsInitFunc,
                                                                                    createChannelFunc);
        ...
        { FOUT_SECTION("Engine::up()")
            ...
            { FOUT_SECTION("Prepare Environment")
                ...
                if (engineData.core.createEngine.executionFilename)
                {
                    ...
                    engineData.environment.set("SupplementaryFilePath",
                                               brahms::os::filenamepath(engineData.execution.fileSysIn), fout);
                ...
                { FOUT_SECTION("Invocation-Level")
                        ...
                        if (engineData.core.createEngine.executionParameters) {
                            ...
                            while(*p) {
                                ...
                                // check to avoid seg fault if wrong data supplied
                                if (!(*q)) {
                                    ferr << E_INVOCATION << "malformed execution parameter string";
                                }
                ...
                engineData.execParsComponent.appendChild(new XMLNode("SupplementaryFilePath",
                                                                     engineData.environment.gets("SupplementaryFilePath").c_str()));
                engineData.execParsComponent.appendChild(new XMLNode("VoiceID", engineData.environment.gets("VoiceID").c_str()));
                engineData.execParsComponent.appendChild(new XMLNode("WorkingDirectory",
                                                                     engineData.environment.gets("WorkingDirectory").c_str()));
            ...
            fout << "Engine is up! (version " << toString(v) <<  ", voice "
                 << unitIndex(engineData.core.getVoiceIndex()) << "/"
