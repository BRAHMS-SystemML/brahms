...
////////////////	THREAD
		...
		{
	/*		The only reason we call this function instead of calling the thread main procedure
			directly is that it allows us to do some in-thread operations without revealing these
			to the user of class Thread. Specifically, we can set the thread state before we
			call thread main, and we can set the Thread state after thread main returns.				*/
		...
		void Thread::terminate(brahms::output::Source& fout, UINT32 t_alreadyWaited)
		{
			...
			while(true)
			{
				...
				//	if already detected as hung, we won't wait the term timeout also
