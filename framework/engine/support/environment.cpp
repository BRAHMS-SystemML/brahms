...
namespace brahms
{
		...
		void Environment::add(string key)
		{
			...
			for(UINT32 n=0; n<nodes->size(); n++)
			{
				if (nodes->at(n)->nodeName() == key)
					ferr << E_EXECUTION_PARAMETERS << "Execution Parameter \"" << key << "\" already exists during add()";
			}
		...
		void Environment::set(string key, string value, brahms::output::Source& fout)
		{
			...
			ferr << E_EXECUTION_PARAMETERS << "Execution Parameter \"" << key << "\" not found during set()";
		}
		...
		void Environment::load(string path, bool insist, brahms::output::Source& fout)
		{
			...
			{
				if (insist) ferr << E_EXECUTION_PARAMETERS << "file not found \"" << path << "\"";
			...
			if (!file) ferr << E_EXECUTION_PARAMETERS << "error opening \"" << path << "\"";
		...
		void Environment::load(const char* path, brahms::xml::XMLNode& nodeFile, brahms::output::Source& fout)
		{
			if (nodeFile.hasChild("ExecutionParameters"))
			{
				...
				for (UINT32 c=0; c<nodes->size(); c++)
				{
					...
					if (node->hasChildNodes())
						ferr << E_EXECUTION_PARAMETERS << "malformed ExecutionParameters in \"" << path << "\"";
					...
					//	modify the tree
					controlExecPars.readonly = false;
					if (nodeExecPars.hasChild(key.c_str())) nodeExecPars.getChild(key.c_str())->nodeText(value.c_str());
					else nodeExecPars.appendChild(new brahms::xml::XMLNode(key.c_str(), value.c_str()));
					fout << key << " = " << value << D_VERB;
					core.execPars.set(key.c_str(), value.c_str());
					controlExecPars.readonly = false;
				}
		...
		DOUBLE Environment::get(const char* key, DOUBLE min, DOUBLE max) const
		{
			...
			DOUBLE value;
			if (nodeText == "NEARLYZERO") value = EFFECTIVELY_ZERO;
			else
			{
				//	numeric
			...
			//	check constraints
			if (value < min)
				ferr << E_EXECUTION_PARAMETERS << "Execution Parameter \"" << key << "\" should be at least " << min;
			if (value > max)
				ferr << E_EXECUTION_PARAMETERS << "Execution Parameter \"" << key << "\" should be not more than " << max;
		...
		UINT32 Environment::getu(const char* key, UINT32 max) const
		{
			...
			if (uvalue != dvalue)
				ferr << E_EXECUTION_PARAMETERS << "Execution Parameter \"" << key << "\" should be an integer";
			return uvalue;
		}
		...
		VUINT32 uexplode(const string delim, const string& data, const char* name)
		{
			...
			for (UINT32 i=0; i<vs.size(); i++)
			{
				...
				if (val == brahms::text::S2N_FAILED || uval != val)
					ferr << E_EXECUTION_PARAMETERS << "Execution Parameter \"" << name << "\" was incorrectly formatted";
				ret.push_back(uval);
			}
		...
		VUINT32 Environment::getulist(const char* key, const UINT32 count) const
		{
			...
			if (ret.size() != count) ferr << E_EXECUTION_PARAMETERS << "Execution Parameter \"" << key << "\" should have three entries";
			return ret;
		}
		...
		bool Environment::getb(const char* key) const
		{
			...
			ferr << E_EXECUTION_PARAMETERS << "Execution Parameter \"" << key << "\" should be a boolean";
		...
		void Environment::assertType(const char* name, char type) const
		{
			...
			switch(type)
			{
				case 'b':
					...
					ferr << E_EXECUTION_PARAMETERS << "Execution Parameter \"" << name << "\" should be boolean (\"0\" or \"1\")";
			}
		}
